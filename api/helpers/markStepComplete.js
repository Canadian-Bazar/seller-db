...
import { stepConfig } from './stepConfig.js'
...
export const markStepCompleteAsync = async (entityId, completedStep, type = 'product') => {
    try {
      ...
      // 🔑 filter only required steps using config
      const requiredSteps = Object.keys(stepConfig[type]).filter(
        step => !stepConfig[type][step].optional
      );
